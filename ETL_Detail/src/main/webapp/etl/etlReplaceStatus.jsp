		...
		<table border="1" cellspacing="0" id="myproc">
			...
			<c:forEach var="list" items="${lists}">
				<tr>
					...
					<td><c:if test="${list.flag==0}">未清洗</c:if>
					<c:if test="${list.flag==1}">正在清洗</c:if>
					<c:if test="${list.flag==2}">清洗完成</c:if>
					</td>
					<td>
					<c:choose>
						<c:when test="${list.day_time==0||list.day_time==null}">没有清洗时间</c:when>
						<c:otherwise>${list.day_time}</c:otherwise>
					</c:choose>
					</td>
